...
export async function promptPrivateVaultPassword(pointer, emailHash, onSuccess = (pwd, data) => alert('Unlocked')) {
    ...
    try {
        ...
        const { json: encryptedContent } = await GD.readJsonByFileId(_privateFileId);

        _privateKey = await CR.deriveKey(pwd, {
...
export async function savePrivateVaultData(data) {
    ...
    await GD.upsertBinaryFile({
        name: _privateFileName,
        fileId: _privateFileId,
...
async function _load() {
    log("vaultCreatePrivateUI._load", "called");

    vaultCreatePrivateUI.createBtn.onClick(_doCreateVaultClick);
    vaultCreatePrivateUI.cancelBtn.onClick(() => window.ScreenManager.goHome());

    _showCreateVaultStatusMessage("Use a strong password to create your personal vault and remember it!", "status-message");
}

async function _unload() {
    log("vaultCreatePrivateUI._unload", "called");
    _showCreateVaultStatusMessage("");
}

function _showCreateVaultStatusMessage(msg, type = "error") {
...
async function _doCreateVaultClick() {
    log("vaultCreatePrivateUI._doCreateVaultClick", "called");
    ...
    try {
        ...
        const result = await _setupPrivateVault(G.userEmail, pwd);

        if (_onGenesisSuccess) {
            await _onGenesisSuccess(result);
        }

        window.ScreenManager.goHome();
    } catch (err) {
        _showCreateVaultStatusMessage(err.message);
    }
}
 ...
 */
async function _setupPrivateVault(userEmail, privatePassword) {
    ...
    _privateKey = await CR.deriveKey(privatePassword, {
        salt: fileSalt,
        iterations: 600000
    });

    // Encrypt returns {iv, data} - we stringify this to store as the file content
    const encryptedObj = await CR.encrypt(JSON.stringify(genesisData), _privateKey);
    ...
    _privateFileId = await GD.upsertBinaryFile({
        name: _privateFileName,
        parentId: parentId,
        content: JSON.stringify(encryptedObj),
        mimeType: "application/octet-stream"
